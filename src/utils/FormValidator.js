...
export default class FormValidator {
  ...
  _showInputError(inputElement, errorMessage) {
    const errorElement = this._form.querySelector(
      this._settings.errorType + inputElement.id
    );
...
const resetValidation = (popup, validationObject) => {
  const inputList = Array.from(
    popup.querySelectorAll(validationObject.inputSelector)
  );
  inputList.forEach((inputElement) => {
  ...
  const errorList = Array.from(
    popup.querySelectorAll(validationObject.popupError)
  );
...
export { resetValidation, resetSubmitBtn };
